...
Guía de arquitectura y convenciones para agentes de IA que trabajen en este proyecto.

## Descripción del proyecto

Sitio web **estático** — una wiki académica sobre conceptos esenciales de metodología de la investigación. No utiliza frameworks de JavaScript ni herramientas de compilación. Es HTML + CSS puro desplegado en Netlify.

El directorio `src/` contiene el scaffold original de TanStack Start pero **no está en uso**. El sitio sirve desde `public/`.
...
```
public/
  index.html   # Wiki completa en un único archivo HTML
  style.css    # Hoja de estilos completa (CSS puro, sin frameworks)
netlify.toml   # publish="public", command="" (sin build step)
...
- **Sin build step**: `netlify.toml` tiene `command = ""` y `publish = "public"`. No ejecutar comandos de compilación.
- **Todo en un archivo HTML**: el contenido es estático; no hay CMS ni generador.
- **Imágenes como SVG inline**: ilustraciones embebidas en `index.html`, sin archivos externos.
- **CSS custom properties**: variables en `:root` dentro de `style.css` controlan toda la paleta de color.
- **Navegación por anclas**: el menú usa `href="#id"` con `scroll-behavior: smooth`.
- **Responsive**: sidebar sticky en escritorio, colapsa a la parte superior en móvil.

## Convenciones CSS

- `.img-placeholder.cN` (N = 1–6): gradientes de color por sección.
- `.wiki-section`: tarjeta visual de cada concepto.
- `.key-points`: bloque destacado con borde izquierdo azul.
- `.subtypes-grid`: grid para la sección de tipos de muestreo.

---

## Scaffold original (no activo)

El directorio `src/` contiene archivos de TanStack Start / React que no se usan. No modificarlos ni ejecutar `npm run build` — el sitio no depende de ellos.
